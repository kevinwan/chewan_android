...
public class CarSeatUtil {
	...
	public void addCar(JSONArray jsa) {
		...
		for (int i = 0; i < jsa.length(); i++) {
			try {
				...
				View childV = mLayoutInflater.inflate(R.layout.item_car_seat,
						null);
				parentV.addView(childV);

				ImageView carLogo = (ImageView) childV.findViewById(R.id.logo);
				ViewUtil.bindNetImage(carLogo,
						JSONUtil.getString(carJo, "carBrandLogo"), "carbiglogo");

				TextView nameT = (TextView) childV.findViewById(R.id.name);
				float size = nameT.getTextSize();

				if (JSONUtil.getString(carJo, "carModel").length() > 5) {
					nameT.setTextSize(TypedValue.COMPLEX_UNIT_PX, size - 6);
				}
				ViewUtil.bindView(childV.findViewById(R.id.name),
						JSONUtil.getString(carJo, "carModel"));

				final String carId = JSONUtil.getString(carJo, "carId");

				int seatCount = JSONUtil.getInt(carJo, "totalSeat");
				totalSeatcount += seatCount;
				LinearLayout seatLayout = (LinearLayout) childV
						.findViewById(R.id.seat_layout);

				for (int j = 0; j < seatCount; j++) {
					final int childPosition = j;
					View seatChild = seatLayout.getChildAt(j);
					ImageView seatI = (ImageView) seatChild
							.findViewById(R.id.seat);
					...
					seatChild.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View v) {
							if (onSeatClickListener != null) {
								onSeatClickListener.onSeatClick(carId,
										childPosition);
							}
						}
					});
		...
		if (onSeatClickListener != null) {
			onSeatClickListener.seatCount(totalSeatcount, totalSeatcount
					- usedSeatCount);
		}
	}

	// public void changeSeatStatus(int groupPosition, int childPosition, String
	// headUrl, int type)
	// {
	// for (int i = 0; i < jsa.length(); i++)
	// {
	...
	// (LinearLayout)childV.findViewById(R.id.seat_layout);
	// for (int j = 0; j < seatcount; j++)
	// {
	// View seatChild = seatLayout.getChildAt(j);
	...
	// headI.setVisibility(View.VISIBLE);
	// seatI.setImageResource(R.drawable.icon_seat_white);
	// ImageLoader.getInstance().displayImage(headUrl, headI,
	// CarPlayValueFix.headOptions);
	// }
	// else
	// {
