...
//                            showToast("不能给自己评论哦");
                    ...
                    ActiveMsgDialog dlg = new ActiveMsgDialog(self, "您还没有登陆是否去登陆？");
        ...
        mListView.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
            ...
            public boolean onItemLongClick(AdapterView<?> parent, View view, final int position, long id) {
                ...
                final JSONObject item = (JSONObject) mJsonAdapter.getItem(position - mListView.getHeaderViewsCount());

                DeleteDialog delDlg = new DeleteDialog(self, R.style.CustomDialog);
                delDlg.setOnDeleteListener(new OnDeleteClickListener() {

                    @Override
                    public void onDelete() {
                        DhNet net = new DhNet(API.CWBaseurl + "/comment/remove?userId=" + user.getUserId() + "&token="
                                + user.getToken());
                ...
                // ActiveMsgDialog dlg = new ActiveMsgDialog(self, "确定删除留言？");
                // dlg.setOnClickResultListener(new OnClickResultListener() {
                //
                // @Override
                // public void onclick() {
                // DhNet net = new DhNet(API.CWBaseurl +
                // "/comment/remove?userId=" + user.getUserId() + "&token="
                // + user.getToken());
                // JSONArray params = new JSONArray();
                // try {
                // params.put(item.getString("commentId"));
                ...
                if (User.getInstance().isLogin()) {
                    ...
                    if (joinT.getText().toString().equals("管理")) {
                        ...
                        // 参与者只能刪除自己的
                        try {
                            if (item.getString("userId").equals(user.getUserId())) {
                                delDlg.show();
                            }
                        } catch (JSONException e) {
                    ...
                    // 沒有登陆
                    ActiveMsgDialog dlg1 = new ActiveMsgDialog(self, "您还没有登陆是否去登陆？");
        ...
        mJsonAdapter = new NetJSONAdapter(API.CWBaseurl + "/activity/" + activityId + "/comment?userId="
                + user.getUserId() + "&token=" + user.getToken(), this, R.layout.listitem_comment);
        mJsonAdapter.addField(new FieldMap("nickname", R.id.tv_nickname) {
            ...
            public Object fix(View itemV, Integer position, Object o, Object jo) {
                ...
                TextView commentView = (TextView) itemV.findViewById(R.id.tv_comment_content);
                String comment = "";
                if (!JSONUtil.getString(jo1, "replyUserId").equals("")) {
                    // 不是回复给发布者的
                    comment = "<font color='#aab2bd'>回复</font>" + "<font color='#5D9CEC'>"
                            + JSONUtil.getString(jo1, "replyUserName") + "</font>" + ": "
                            + JSONUtil.getString(jo1, "comment");
                } else {
                    comment = JSONUtil.getString(jo1, "comment");
                }
                commentView.setText(Html.fromHtml(comment));
                RoundImageView headI = (RoundImageView) itemV.findViewById(R.id.imgView_avatar);
                ViewUtil.bindNetImage(headI, JSONUtil.getString(jo1, "photo"), "head");
    ...
    public void getData() {
        DhNet net = new DhNet(API.CWBaseurl + "/activity/" + activityId + "/info?userId=" + user.getUserId()
                + "&token=" + user.getToken());
    ...
    private void bindHeadView(final JSONObject headJo) {
        ...
        CarPlayUtil.bindDriveAge(createrJo, (ImageView) headV.findViewById(R.id.car_logo),
                (TextView) headV.findViewById(R.id.drive_age));
        RoundImageView headI = (RoundImageView) headV.findViewById(R.id.head);
        ViewUtil.bindNetImage(headI, JSONUtil.getString(createrJo, "photo"), "head");
        ...
        ViewUtil.bindView(headV.findViewById(R.id.empty_seats), JSONUtil.getString(headJo, "seatInfo"));
        ...
        final PicLayoutUtil util = new PicLayoutUtil(self, picJsa, 5, pivlayout, piclayoutWidth);
        util.addMoreChild();
        ...
        PicLayoutUtil headUtil = new PicLayoutUtil(self, headJsa, 5, headlayoutV, headlayoutWidth);
    ...
    public void comment() {
        ...
        DhNet net = new DhNet(API.CWBaseurl + "/activity/" + activityId + "/comment?userId=" + user.getUserId()
                + "&token=" + user.getToken());
        try {
    ...
    private void attention() {
        DhNet net = new DhNet(API.CWBaseurl + "/activity/" + activityId + "/subscribe?userId=" + user.getUserId()
                + "&token=" + user.getToken());
        net.doPostInDialog(new NetTask(self) {
    ...
    private void cancleattention() {
        DhNet net = new DhNet(API.CWBaseurl + "/activity/" + activityId + "/unsubscribe?userId=" + user.getUserId()
                + "&token=" + user.getToken());
        net.doPostInDialog(new NetTask(self) {
    ...
    private void joinActive(int seatCount) {
        DhNet net = new DhNet(API.CWBaseurl + "/activity/" + activityId + "/join?userId=" + user.getUserId()
                + "&token=" + user.getToken());
    ...
    private void isAuthen() {
        showProgressDialog("");
        User user = User.getInstance();
        DhNet mDhNet = new DhNet(API.availableSeat + user.getUserId() + "/seats?token=" + user.getToken());
            ...
            public void doInUI(Response response, Integer transfer) {
                ...
                if (response.isSuccess()) {
                    ...
                    try {
                        ...
                        if (user.getIsAuthenticated() == 1) {
                            hidenProgressDialog();
                            CarSeatSelectDialog dialog = new CarSeatSelectDialog(self, activityId);
            ...
            if (isMember == 1) {
                ...
                try {
                    ...
                    for (int i = 0; i < headJsa.length(); i++) {
                        ...
                        if (!JSONUtil.getString(headJo, "userId").equals(User.getInstance().getUserId())) {
                            newjsa.put(headJo);
                        }
                    }
                    headlayoutV.removeAllViews();
                    PicLayoutUtil headUtil = new PicLayoutUtil(self, newjsa, 5, headlayoutV, headlayoutWidth);
                    headUtil.setHeadMaxCount(6);
                    headUtil.AddChild();
                } catch (JSONException e) {
                    // TODO Auto-generated catch block
    ...
    public JSONObject getShareContent(JSONObject jo) {
        ...
        try {
            ...
            String shareTitle = JSONUtil.getString(userJo, "nickname") + "邀请您参加"
            ...
            String shareContent = "开始时间: " + startTime + "\n目的地: " + JSONUtil.getString(jo, "location") + "\n费用: "
                    + JSONUtil.getString(jo, "pay");
            String shareUrl = API.share + "share.html?code=" + JSONUtil.getString(jo, "activityId");
