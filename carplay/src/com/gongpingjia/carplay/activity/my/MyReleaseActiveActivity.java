	...
	public void initView() {
		...
		listV.setOnEmptyDataListener(new OnEmptyDataListener() {
			...
			public void onEmpty(boolean showeEptyView) {
				...
				findViewById(R.id.btn_submit).setVisibility(
						showeEptyView ? View.VISIBLE : View.GONE);
		...
		adapter = new MyReleaseActiveAdapter(API.CWBaseurl + "/user/"
				+ user.getUserId() + "/post?userId=" + user.getUserId()
				+ "&token=" + user.getToken(), self,
				R.layout.item_myrelease_active);
