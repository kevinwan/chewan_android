...
public class CarTypeSelectActivity extends CarPlayBaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mPopView = LayoutInflater.from(this).inflate(
				R.layout.pop_brand_details, null);
		...
		mBrandHeader = LayoutInflater.from(self).inflate(
				R.layout.listitem_car_brand, null);
			...
			public void doInUI(Response response, Integer transfer) {
				...
				if (response.isSuccess()) {
					...
					mBrandAdapter = new BrandAdapter(
							CarTypeSelectActivity.this, mDatas);
					...
					public void doInUI(Response response, Integer transfer) {
						...
						if (response.isSuccess()) {
							...
							ArrayAdapter<String> adapter = new ArrayAdapter<String>(
									self, R.layout.listitem_brand,
									R.id.tv_brand_name, strs);

							// 更新list view header数据
							ImageView image = (ImageView) mBrandHeader
									.findViewById(R.id.imgView_car_logo);
							ImageLoader.getInstance().displayImage(
									mDatas.get(position).getUrl(), image,
									CarPlayValueFix.optionsDefault);
							TextView text = (TextView) mBrandHeader
