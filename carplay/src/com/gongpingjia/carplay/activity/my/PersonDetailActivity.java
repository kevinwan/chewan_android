...
public class PersonDetailActivity extends CarPlayBaseActivity implements OnClickListener {
    ...
    public void initView() {
        ...
        listV.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                JSONObject jo = null;
                if (type == RELEASE) {
                    jo = (JSONObject) releaseAdapter.getItem(position - listV.getHeaderViewsCount());
                } else if (type == ATTENTION) {
                    jo = (JSONObject) attentionAdapter.getItem(position - listV.getHeaderViewsCount());
                } else {
                    jo = (JSONObject) joinAdapter.getItem(position - listV.getHeaderViewsCount());
                }
        ...
        gallery.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                for (int i = 0, len = albumPhotosJsa.length(); i < len; i++) {
                    try {
                        imgUrls[i] = albumPhotosJsa.getJSONObject(i).getString("thumbnail_pic");
    ...
    private void getPersonData() {
        DhNet net = new DhNet(API.CWBaseurl + "/user/" + userId + "/info?userId=" + user.getUserId() + "&token="
                + user.getToken());
            ...
            public void doInUI(Response response, Integer transfer) {
                if (response.isSuccess()) {
                    ...
                    if (JSONUtil.getString(jo, "role").equals("官方用户")) {
                        ...
                        headV.findViewById(R.id.layout_sex).setVisibility(View.GONE);
                    } else {
                        headV.findViewById(R.id.car_logo).setVisibility(View.VISIBLE);
                        headV.findViewById(R.id.drive_age).setVisibility(View.VISIBLE);
                        headV.findViewById(R.id.layout_sex).setVisibility(View.VISIBLE);
                        CarPlayUtil.bindDriveAge(jo, (ImageView) headV.findViewById(R.id.car_logo),
                                (TextView) headV.findViewById(R.id.drive_age));
                        CarPlayUtil.bindSexView(JSONUtil.getString(jo, "gender"), headV.findViewById(R.id.layout_sex));
                    }
                    ...
                    ViewUtil.bindView(headV.findViewById(R.id.age), JSONUtil.getString(jo, "age"));
    ...
    private void buildAdapter() {
        releaseAdapter = new MyReleaseActiveAdapter(API.CWBaseurl + "/user/" + userId + "/post?userId="
                + user.getUserId() + "&token=" + user.getToken(), self, R.layout.item_myrelease_active);
        ...
        attentionAdapter = new ActiveAdapter(API.CWBaseurl + "/user/" + userId + "/subscribe?userId="
                + user.getUserId() + "&token=" + user.getToken(), self, R.layout.item_active_list);

        attentionAdapter.fromWhat("data");
        attentionAdapter.showNext();

        joinAdapter = new ActiveAdapter(API.CWBaseurl + "/user/" + userId + "/join?userId=" + user.getUserId()
    ...
    public void onClick(View v) {
        ...
        case R.id.head:
            if (mHeadImgUrl != null) {
                String[] imgUrls = new String[] { mHeadImgUrl };
                Intent it = new Intent(self, ImageGallery.class);
                it.putExtra("imgurls", imgUrls);
                startActivity(it);
            }
    ...
    private void bindEmptyView(View emptyV, final int type) {
        ...
        case ATTENTION:
            ViewUtil.bindView(findViewById(R.id.icon_msg), R.drawable.icon_shoucang);
