...
public class DhNet
{
    ...
    public DhNet execuse(NetTask task)
    {
        ...
        // 在加载前加载缓存
        if (cachePolicy == CachePolicy.POLICY_CACHE_ONLY || cachePolicy == CachePolicy.POLICY_CACHE_AndRefresh
            ...
            public void run()
            {
                ...
                if (hasNet)
                {
                    ...
                    try
                    {
                        ...
                        Log.d("duohuo_DhNet", DhNet.this.url + " method: " + method + " params: " + params
                            + " result: " + result);
                        Response response = new Response(result);
                        ...
                        if (golbal != null)
                        {
                            golbal.hanlder(DhNet.this.task.mContext, response);
                        }
     ...
     * 如果访问已经启动 如果isInterrupt 为 true 则访问会被打断 , 否则 会线程继续运行 取消时必定会调用 task 的onCancel方法
     * 
     * @return
     */
    ...
    /**
     * 文件上传, 支持大文件的上传 和文件的上传进度更新 task inui response 的bundle参数 uploading true 上传中,false 上传完毕 ; process 上传进度 0-100 cancel
     * 方法可以取消上传
     * 
     * @param url
     * @param name
                ...
                try
                {
                    if (params != null)
                    {
                        for (String key : params.keySet())
                        {
                            ...
                            {
                                mulentity.addPart(key,
                                    new StringBody(params.get(key).toString(), Charset.forName("UTF-8")));
                            }
                        }
                    }
                }
    ...
    public void upload(final FileInfo fileInfo, NetTask task)
    {
        ...
        this.feture = executeRunalle(new Runnable()
        {
            public void run()
            {
                try
                {
                    String result = PostFile.getInstance().post(getUrl(), params, fileInfo);
                    Log.d("duohuo_DhNet", DhNet.this.url + " method:" + method + " params: " + params + " result: "
                        + result);
                    Response response = new Response(result);
                    response.isCache(false);
                    // 获取错误码
