...
public class INetRefreshAndMorelistView extends ListView implements OnScrollListener
{
    ...
    {
        headView = LayoutInflater.from(context).inflate(R.layout.list_refresh_head, null);
    ...
    public void onScroll(AbsListView view, int firstVisiableItem, int allVisiable, int allItems)
    {
        ...
        if (this.footView != null)
        {
            ...
            {
                if (allItems - (firstVisiableItem + allVisiable) <= leaveCount && !isLoadingMore)
                {
                    if (this.getAdapter() instanceof HeaderViewListAdapter)
                    {
                        HeaderViewListAdapter headadapter = (HeaderViewListAdapter)this.getAdapter();
                        if (headadapter.getWrappedAdapter() instanceof INetAdapter)
                        {
                            ...
                            if (trueAdapter.hasMore())
                            {
                                ...
                                if (moreLoadSuccessCallBack == null)
                                {
                                    ...
                                    trueAdapter.setOnLoadSuccess(this.moreLoadSuccessCallBack);
                                }
        ...
        if (isRefreshable)
        {
            switch (event.getAction())
            {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (state != STATE_REFRESHING && isRecored && state != STATE_LOADING)
                    {
                        ...
                        {
                            setSelection(0);
                            if (((tempY - startY) / RATIO < refreshHeight) && (tempY - startY) > 0)
                            {
                    ...
                    else if (footView != null && state != STATE_MORE_LOADING && isBottomRecord
                        && state != STATE_LOADING)
                    {
                        if (state == STATE_DONE)
                            state = STATE_MORE_OK;
                        if (state == STATE_RELEASE_To_More)
                        {
                            ...
                            if (((startY - tempY) / RATIO < moreHeight) && (startY - tempY) > 0)
                            {
     ...
     * 
     * @param state
     * @param view 当 refresh 时view 为headview 当 加载下页时 view 为headView
     */
...
*/
     ...
     * 
     * @param footView
     * @param leaveCount 剩下多少时开始自动加载
     */
    public void setMoreView(View footView, Integer leaveCount)
    {
        // footView.measure(View.MeasureSpec.UNSPECIFIED, View.MeasureSpec.UNSPECIFIED);
